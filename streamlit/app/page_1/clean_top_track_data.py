...
def top_track_dataframe(response):
    ...
    df['album'] = [response.json().get('tracks', [])[i]['album']['name'] 
                   for i in range(len(df))]
    df['release_date'] = [response.json().get('tracks', [])[i]['album']['release_date']
                          for i in range(len(df))]
    df['album_id'] = [response.json().get('tracks',[])[i]['album']['id']
                      for i in range(len(df))]
    df['album_image'] = [response.json().get('tracks', [])[i]['album']['images'][0]['url']
                         for i in range(len(df))]
